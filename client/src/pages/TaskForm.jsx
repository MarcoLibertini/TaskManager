        ...
        try {
            if(!params.id) {
               ...
               // Hacemos una petición put a la API para actualizar una tarea
                const response = await axios.put(`${import.meta.env.VITE_API_URL}/api/tasks/${params.id}`, { 
                    title, 
                    description 
                })
    ...
    useEffect(() => {
        async function fetchTask() {
            if (params.id && params.id !== 'undefined') {
                try {
                    const response = await axios.get(`${import.meta.env.VITE_API_URL}/${params.id}`)
...

                    {
                        ...
                        onClick={async () => {
                            try {
                                ...
                                const response = await axios.delete(`${import.meta.env.VITE_API_URL}/api/tasks/${params.id}`)
